...
def get_data_path(agrs, train_mode: str):
    ...
    if args.data_model == "Domain":
        ...
        if args.is_attack == "clearn":
            ...
            return os.path.join(
                args.data_path,
                "xval_{}_ner{}".format(args.is_attack, text),
                "ner_{}_{}{}.json".format(train_mode, args.N, text).replace("_", replace_text)
            )

    if args.data_model == "Snips":
        if train_mode == "dev":
            train_mode = "valid"
        text = "_shot_5" if args.K == 5 else ""
        replace_text = "-" if args.K == 5 else "_"
        if args.is_attack == "clearn":
            return os.path.join(
                args.data_path,
                "xval_snips{}".format(text),
                "snips_{}_{}{}.json".format(train_mode, args.N, text).replace("_", replace_text)
            )
        else:
            return os.path.join(
                args.data_path,
                "xval_{}_snips{}".format(args.is_attack, text),
                "snips_{}_{}{}.json".format(train_mode, args.N, text).replace("_", replace_text)
            )
...
def train(args):
    ...
    ma_entity_len = max(len(train_corpus.acount), len(valid_corpus.acount), len(test_corpus.acount))

    learner = Learner(args.bert_model, args.freeze_layer, logger, args.lr, args.warmup_prop, args.max_train_steps, ma_entity_len, args=args)
    ...
    for step in trange(0, int(args.max_train_steps), desc="Steps: ", disable=False):
        ...
        span_loss, type_loss, loss = learner.forward(batch_query, batch_support, progress, train_corpus)
...
def evaluate(args):
    ...
    learner = Learner(args.bert_model, args.freeze_layer, logger, args.lr, args.warmup_prop,
                      args.max_train_steps, ma_entity_len, model_dir=args.model_dir, args=args)
...
if __name__ == "__main__":
    ...
    # dataset setting 
    parser.add_argument("--data_path", type=str, default="episode-data", help="path of datasets")
    parser.add_argument("--data_model", type=str, default="Domain", help="Snips or Domain")
    parser.add_argument("--mode", type=str, default="Domain")
    parser.add_argument("--types_path", type=str, default="episode-data/entity_types_domain.json", help="path of entities types")
    parser.add_argument("--N", type=int, default=2, help="index of test dataset, ontonotes: 4, wnut: 3, gum: 2, conll: 1")
    ...
    parser.add_argument("--is_attack", type=str, default="clearn", help="clearn or attack")
    ...
    parser.add_argument("--choice_number", type=int, default=5, help="choice label_word top n")
